package com.xander.threadtest.executor;
...
public class MyExecutor {
    ...
    /** 重要 最大线程数量  不是核心线程数量个数 + 等待队列的任务个数的 最大值
     * 而是 线程池允许运行的 最大线程数量
     */
    private static final int MAX_THREAD_COUNT = 5;            //线程池最大线程数量，同一时刻线程池（允许运行-这点很重要）的最大数量，如果大于该数量，执行拒绝策略
    ...
    /**
     * 线程池创建，等待队列参数中使用如{@link #mWaitQueue}
     */
    private static final int WAIT_THREAD_COUNT = 2;           //等待队列线程数量

    /**
     * 等待线程队列:有界的任务队列
     * 当有新的任务需要执行，调用execute（runnable）;
     * 1.当线程池中线程数量小于核心数量CORE_THREAD_COUNT，则优先创建线程
     * 2.当线程池中线程数量大于核心数量，则会将任务加入等待队列，等待队列满了之后才会根据最大线程数量判断是否创建新线程执行该任务
...
//    private ArrayBlockingQueue<Runnable> mWaitQueue = new ArrayBlockingQueue<Runnable>(WAIT_THREAD_COUNT);
    public LinkedBlockingQueue<Runnable> mWaitQueue = new LinkedBlockingQueue<Runnable>(WAIT_THREAD_COUNT);
